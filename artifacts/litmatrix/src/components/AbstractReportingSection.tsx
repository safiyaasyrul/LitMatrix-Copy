...
function assessFromAbstract(record: SLRRecord, protocol: SLRProtocol): AbstractReportingAssessment {
  ...
  const result: AbstractReportingAssessment = {
    ...
    evidenceNotes: "Initial abstract-level screen; reviewer confirmation required. “Unclear” means not reported in the available abstract.",
  };
...
${includedRecords.map((record) => `recordId: ${record.id}\ntitle: ${record.title}\nabstract: ${record.abstract || "Not reported"}`).join("\n\n")}`;
      ...
      setErrorMessage(error?.message || "The abstract-reporting assessment could not be generated. A conservative local draft was created for reviewer confirmation.");
    } finally {
      setEvaluating(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {!includedRecords.length && <div className="rounded-xl border border-amber-200 bg-amber-50 p-6 text-center text-sm text-amber-900"><FileText className="mx-auto mb-2 h-7 w-7" />Reviewer-confirmed included records are required before appraisal.</div>}
      {includedRecords.length > 0 && !assessments.length && <div className="rounded-xl border border-slate-200 bg-white p-6 text-center text-sm text-slate-600">Draft the checklist to begin, then confirm each row against the available abstract.</div>}
        ...
        {includedRecords.map((record) => {
          ...
          return <section key={record.id} className="rounded-xl border border-slate-200 bg-white p-5 shadow-xs">
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <button onClick={() => setEditingId(editing ? null : record.id)} className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50">{editing ? "Done" : "Confirm / edit"}</button>
            </div>
      ...
      {onNavigateToScreening && includedRecords.length === 0 && <button onClick={onNavigateToScreening} className="flex items-center gap-2 text-xs font-semibold text-indigo-700 hover:text-indigo-900"><Check className="h-4 w-4" />Return to reviewer-confirmed screening</button>}
    </div>
  );
}