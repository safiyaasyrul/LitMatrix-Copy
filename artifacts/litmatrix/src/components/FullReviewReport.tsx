...
const getEvidenceLandscape = (
  ...
  const themeDefinitions = [
    ...
    {
      label:
        "Technologies, interventions, and decarbonization",
      terms: [
        "fuel",
        "vessel",
        "propulsion",
    ...
    {
      label:
        "Performance, efficiency, and reported outcomes",
      terms: [
        "performance",
        "efficiency",
        "reduction",
  ...
  const themes = records.map((record) => {
    const searchableText =
      `${record.title} ${record.abstract || ""}`.toLowerCase();
...
const summarizeLandscape = (
  counts: LandscapeCount[],
  limit = 4
) =>
  counts
    .slice(0, limit)
...
export default function FullReviewReport({
  ...
  /*
   * Evidence landscape is calculated ONLY from the
   * final included records.
   *
   * There is no artificial study limit.
   */
  const evidenceLandscape =
    getEvidenceLandscape(
      includedRecords
    );

  const recordGroundedRationale =
    includedRecords.length > 0
      ? `This review examines ${
        ...
        } included records. The record-level evidence is concentrated in ${
          summarizeLandscape(
            evidenceLandscape.themeCounts
          ) ||
          "the themes reported in the included literature"
        }, covering the methods, technologies, and outcomes described by those records.`
      : protocol.introductionRationale ||
        `This review examines evidence relevant to ${
          protocol.title ||
          "the defined topic"
        }.`;
  ...
  const getFrameworkNarrative = () => {
    ...
    if (fw === "SPIDER") {
      const s =
        protocol.objectivesSPIDER?.sample ||
        protocol.objectivesPICO.population;

      const pi =
        protocol.objectivesSPIDER
          ?.phenomenonOfInterest ||
        protocol.objectivesPICO.intervention;

      const d =
        protocol.objectivesSPIDER?.design ||
        "qualitative thematic investigations";

      const e =
        protocol.objectivesSPIDER?.evaluation ||
        protocol.objectivesPICO.outcomes;

      const r =
        protocol.objectivesSPIDER?.researchType ||
  ...
  const getAbstractContent = () => {
    ...
    const obj =
      `This systematic review aimed to ${objectives
        .map((o) =>
          o
            .toLowerCase()
            .replace(/^to\s+/, "")
        ...
        .join(
          ", and to "
        )}, addressing three principal research questions: ${questions
        .map(
          (q, i) =>
            `RQ${i + 1} (${q.replace(
              /^RQ\d+:\s*/,
    ...
    const searchDbs =
      protocol.searchStrategies
        .map((s) => s.database)
        .join(", ") ||
      "major electronic bibliographic databases";

    const meth =
      `The review draws on records from ${searchDbs}. Screening decisions follow predefined eligibility criteria, and the included evidence is organized for narrative and thematic synthesis.`;

    const res =
      `${includedRecords.length} records were retained for synthesis from ${
        counts.afterDedup ||
        counts.screened ||
        includedRecords.length
      } records after deduplication. Publication years were distributed as follows: ${
        summarizeLandscape(
          evidenceLandscape.yearCounts
        ) ||
        "no publication-year pattern was available"
      }. The most represented source venues were ${
        summarizeLandscape(
          evidenceLandscape.sourceCounts
        ) ||
        "not specified"
      }, and the dominant record-level themes were ${
        summarizeLandscape(
          evidenceLandscape.themeCounts
        ) ||
        "not specified"
    ...
    const concl =
      `The included literature presents a narrative and thematic evidence base organized around the reported methods, technologies, and outcomes. Interpretation is anchored to the findings and publication characteristics of the included records.`;
  ...
  const generateFullMarkdown = () => {
    ...
    md +=
      `### 1.1 Scientific Rationale and Motivation for Conducting the Review\n`;
    ...
    md +=
      `### 1.2 Review Objectives and Research Questions\n`;

    const questionsParagraph =
      questions
        .map(
          (q, i) =>
            `Specifically, research question ${
              i + 1
            } investigates ${q.replace(
              /^RQ\d+:\s*/,
              ""
            )}`
        )
    ...
    md +=
      `### 2.1 Study Formulation and Scope Definition\n`;
    ...
    const excText =
      protocol.eligibilityCriteria.exclusion.join(
        ", "
      );

    md += `Records were eligible for inclusion if they satisfied predefined criteria encompassing ${incText}. Records were excluded if they exhibited ${excText}. The planned synthesis grouping strategy follows ${
      protocol.eligibilityCriteria
        .groupingForSynthesis ||
      "thematic and technological categorization"
    }.\n\n`;
    ...
    md +=
      `### 2.4 Selection Process and Evidence Status\n`;

    md += `Screening decisions were aligned with predefined eligibility criteria, and included records were organized for narrative and thematic synthesis. No full-text retrieval or full-text eligibility assessment is represented in this report unless such activity was actually recorded in the review workflow.\n\n`;
    ...
    md +=
      `### 3.1 Study Selection and Flow of Evidence\n`;

    md += `${counts.uploaded || counts.identifiedDb || 0} records were uploaded, including ${counts.duplicatesRemoved || 0} duplicates recorded as removed. After deduplication, ${counts.afterDedup || counts.screened || 0} records remained, with ${includedRecords.length} included and ${
      (counts.afterDedup ||
        counts.screened ||
        0) -
      includedRecords.length
    } excluded. The results describe the records retained by the configured screening criteria.\n\n`;

    md +=
      `### 3.2 Comprehensive Screening Decision Table (Table 1)\n\n`;
    ...
    screenedRecords.forEach(
      (record) => {
        const justification =
          screening[record.id]?.reason ||
          "No screening justification was supplied for this record.";

        md += `| ${getArticleRecord(
          record
        ).replace(/\|/g, "/")} | ${getScreeningStatus(
          record
    ...
    md +=
      `### 3.3 Evidence Landscape: Publication Trends, Source Venues, and Themes\n\n`;

    md += `The ${includedRecords.length} included records were distributed across the following publication years: ${
      summarizeLandscape(
        evidenceLandscape.yearCounts
      ) ||
      "no publication-year pattern was available"
    }. The represented source venues were ${
      summarizeLandscape(
        evidenceLandscape.sourceCounts
      ) || "not specified"
    }. Record-level text most frequently addressed ${
      summarizeLandscape(
        evidenceLandscape.themeCounts
    ...
    evidenceLandscape.yearCounts.forEach(
      (item) => {
    ...
    evidenceLandscape.sourceCounts.forEach(
      (item) => {
        md += `| ${item.label.replace(
          /\|/g,
          "/"
        )} | ${item.count} |\n`;
      }
    ...
    evidenceLandscape.themeCounts.forEach(
      (item) => {
        md += `| ${item.label.replace(
          /\|/g,
          "/"
    ...
    md +=
      `### 3.4 Evidence Synthesis Grouped by Study Characteristics and Shared Author Similarities\n\n`;

    synthesis.subtopics.forEach(
      (sub) => {
        md += `#### ${sub.title}\n${sub.prose}\n\n`;
      }
    );
    ...
    md +=
      `### 4.1 Principal Findings, Category Clusters, and Cross-Author Synthesis\n${discussion.item23aGeneralInterpretation}\n\n`;

    md +=
      `### 4.2 Methodological Characteristics of Included Evidence\n${discussion.item23bLimitationsOfEvidence}\n\n`;

    md +=
      `### 4.3 Review Methodological Context\n${discussion.item23cLimitationsOfReviewProcess}\n\n`;
  ...
  const handleDownload = () => {
    ...
    a.download =
      "Systematic_Literature_Review_Manuscript.md";
...
<body>
  ...
  <div class="abstract-box">
    <h2 style="margin-top: 0; border-bottom: none; font-size: 13pt;">
      Abstract
    </h2>
  ...
  ${
    protocol.backgroundContext &&
    includedRecords.length === 0
      ? `<p>In theoretical and domain context, ${protocol.backgroundContext}</p>`
      : ""
  }
  ...
  ${
    protocol.knowledgeGap &&
    includedRecords.length === 0
      ? `<p>Regarding the existing literature gap, ${protocol.knowledgeGap}</p>`
      : ""
  }
  ...
  <p>
    The overarching objective of this investigation is
    ${objectives
      .map(
        (obj) =>
          `to ${obj
      ...
      .join(", as well as ")}.
    In addressing this mandate, the systematic review addresses
  ...
  <p>
    Records were eligible for inclusion if they satisfied predefined
    criteria encompassing
    ${protocol.eligibilityCriteria.inclusion.join(
      ", "
    )}.
    Records were excluded if they exhibited
    ${protocol.eligibilityCriteria.exclusion.join(
      ", "
    )}.
    Synthesis grouping was structured around
    ${
      protocol.eligibilityCriteria
        .groupingForSynthesis ||
      "thematic technological categories"
    }.
  </p>
  ...
  <p>
    Systematic search strings were executed across major academic
    databases
    (${protocol.searchStrategies
      .map((s) => s.database)
      .join(", ")}).
    Search strategies combined controlled vocabulary terms,
    Boolean logic, and field constraints.
  </p>
  ...
  <p>
    Screening decisions were aligned with predefined eligibility
    criteria, and included records were organized for narrative
    and thematic synthesis. No full-text retrieval or full-text
    eligibility assessment is represented unless such activity
    was actually recorded in the review workflow.
  </p>
  ...
  <p>
    Uploaded records:
    ${counts.uploaded || counts.identifiedDb || 0}.
    After deduplication:
    ${counts.afterDedup || counts.screened || 0}.
    Included:
    ${includedRecords.length}.
    Excluded:
    ${
    ...
    The results describe the records retained by the configured
    screening criteria.
  </p>

  <h3>
    3.2 Evidence Landscape: Publication Trends, Source Venues, and Themes
  </h3>

  <p>
    The ${includedRecords.length} included records were distributed
    across the following publication years:
    ${
      summarizeLandscape(
        evidenceLandscape.yearCounts
      ) ||
  ...
  <h3>
    3.3 Comprehensive Screening Decision Table (Table 1)
  </h3>

  <div class="table-caption">
    Table 1: Article information, screening status, and academic
  ...
  <table>
    <thead>
      <tr>
        ...
        <th>
          Academic Screening Justification
        </th>
      </tr>
    </thead>
  ...
  <h3>
    3.4 Evidence Synthesis Grouped by Study Characteristics and Author Similarities
  </h3>
  ...
  ${includedRecords
    ...
    a.download = `${
      (
        protocol.title ||
        "Systematic_Literature_Review_Manuscript"
      )
        .replace(
          /[^a-zA-Z0-9]/g,
          "_"
  ...
  return (
    <div
      id="full-review-report-container"
      className="space-y-6"
    >
      {/* ======================================================
          ACTION BAR
      ...
      <div className="bg-white border border-slate-200 p-6 rounded-xl shadow-xs flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Authoritative systematic review manuscript
            with a structured academic abstract,
            narrative evidence synthesis, screening
            documentation, publication trends, and
      ...
      <article className="bg-white border border-slate-200 p-8 sm:p-12 rounded-xl shadow-xs font-sans space-y-8 max-w-4xl mx-auto print:border-none print:shadow-none print:p-0">
        ...
        <section className="bg-slate-50/80 border border-slate-200 p-6 sm:p-8 rounded-xl space-y-4">
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-700 leading-relaxed font-sans text-justify">
            <p>
              <strong className="font-mono font-bold text-slate-900 uppercase text-[11px] mr-1.5">
                Background:
              </strong>
        ...
        <section className="space-y-4">
          ...
          <div className="space-y-2">
            <h3 className="font-bold text-slate-900 text-sm font-mono">
              1.1 Scientific Rationale and Motivation
            ...
            {protocol.backgroundContext &&
              includedRecords.length === 0 && (
                <p className="text-xs sm:text-sm text-slate-700 leading-relaxed font-sans text-justify">
                  In theoretical and domain context,{" "}
                  {
                    protocol.backgroundContext
                  }
                </p>
            ...
            {protocol.knowledgeGap &&
              includedRecords.length === 0 && (
                <p className="text-xs sm:text-sm text-slate-700 leading-relaxed font-sans text-justify">
                  Regarding the existing literature gap,{" "}
                  {protocol.knowledgeGap}
                </p>
              )}
          </div>
          ...
          <div className="space-y-2 pt-2">
            ...
            <p className="text-xs sm:text-sm text-slate-700 leading-relaxed font-sans text-justify">
              ...
              In addressing this mandate, the review investigates
              three core research questions:{" "}
              {questions
                .map(
                  (q, i) =>
        ...
        <section className="space-y-4">
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-700 leading-relaxed">
            ...
            <p className="text-justify">
              Records were eligible for inclusion if they satisfied
              predefined criteria encompassing{" "}
              {protocol.eligibilityCriteria.inclusion.join(
                ", "
              )}
              . Records were excluded if they exhibited{" "}
            ...
            <p className="text-justify">
              Systematic search strings were executed across major
              academic databases (
              {protocol.searchStrategies
                .map((s) => s.database)
                .join(", ")}
              ). Search strategies combined controlled vocabulary
              terms, Boolean logic, and field constraints.
            </p>
            ...
            <p className="text-justify">
              Screening decisions were aligned with predefined
              eligibility criteria, and included records were
              organized for narrative and thematic synthesis. The
              present workflow does not represent full-text
              retrieval or full-text eligibility assessment unless
              such activity was actually recorded.
            </p>
          </div>
        </section>
        ...
        <section className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <p className="text-xs sm:text-sm text-slate-700 leading-relaxed text-justify">
              ...
              {(counts.afterDedup ||
                ...
                0) -
                includedRecords.length}
              . The results describe the records retained by the
              configured screening criteria.
            </p>
          ...
          <div className="space-y-3 pt-2">
            <h3 className="font-bold text-slate-900 text-sm font-mono">
              3.2 Evidence Landscape: Publication Trends,
              Source Venues, and Themes
            </h3>

            <p className="text-xs sm:text-sm text-slate-700 leading-relaxed text-justify">
              The {includedRecords.length} included records were
              distributed across the following publication years:{" "}
              {summarizeLandscape(
                evidenceLandscape.yearCounts
              ) ||
                "no publication-year pattern was available"}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {[
                ...
                ([label, values]) => (
                  ...
                  >
                    <div className="bg-slate-50 px-3 py-2 text-[10px] font-mono font-bold text-slate-800">
                      {label as string}
                    </div>

                    <div className="divide-y divide-slate-100">
                      {(
                        values as LandscapeCount[]
                      ).map(
                        (item) => (
                          <div
                            key={item.label}
          ...
          <div className="space-y-3 pt-4">
            <h3 className="font-bold text-slate-900 text-sm font-mono">
              3.4 Evidence Synthesis Grouped by Study
              Characteristics and Author Similarities
            </h3>
            ...
            {synthesis.subtopics.length ===
              0 && (
              <p className="text-xs sm:text-sm text-slate-500 italic">
                No narrative synthesis has been generated yet.
              </p>
            )}
          </div>
        </section>
        ...
        <section className="space-y-4">
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-700 leading-relaxed">
            <div>
              <h3 className="font-bold text-slate-900 text-xs font-mono mb-1">
                4.1 Principal Findings, Category Clusters,
                and Cross-Author Synthesis
              </h3>
            ...
            <div>
              <h3 className="font-bold text-slate-900 text-xs font-mono mb-1">
                4.2 Methodological Characteristics of
                Included Evidence
              </h3>
            ...
            <div>
              <h3 className="font-bold text-slate-900 text-xs font-mono mb-1">
                4.4 Practical Implications and Future
                Research Directions
              </h3>
          ...
          <div className="space-y-2 text-xs text-slate-600 font-sans leading-relaxed">
            ...
            {includedRecords.length ===
              ...
              <p className="text-slate-500 italic">
                No included studies are currently available.
              </p>
            )}
          </div>
        </section>
