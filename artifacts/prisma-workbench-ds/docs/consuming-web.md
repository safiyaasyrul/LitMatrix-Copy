# Consuming PRISMA 2020 Workbench Design System in web apps

Read `artifacts/prisma-workbench-ds/docs/AGENTS.md` first. This guide covers
...
`artifacts/prisma-workbench-ds/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/prisma-workbench-ds/styles.css";
```
...
`node_modules/@workspace/prisma-workbench-ds/src/components` to `content`.
...
```tsx
import { Button } from "@workspace/prisma-workbench-ds/components/ui/button";
import { cn } from "@workspace/prisma-workbench-ds/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/prisma-workbench-ds/hooks/use-toast";
```
...
`@workspace/prisma-workbench-ds/components/ui/button`. Run the app's typecheck
