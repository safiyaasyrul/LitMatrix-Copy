# Migrating Expo UI to PRISMA 2020 Workbench Design System

Read `artifacts/prisma-workbench-ds/docs/AGENTS.md` and
`artifacts/prisma-workbench-ds/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/prisma-workbench-ds/lib/native-theme` and
  `@workspace/prisma-workbench-ds/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/prisma-workbench-ds/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/prisma-workbench-ds` and retains only
